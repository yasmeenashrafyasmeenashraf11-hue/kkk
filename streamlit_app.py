...
st.title("💬 Chatbot") st.write( "This is a simple chatbot that uses Googles Gemini model to generate responses. " "To use this app, you need to provide a Google AI API key, which you can get from " "Google AI Studio." )

Configure Gemini API
if "gemini_model" not in st.session_state: google_api_key = st.text_input("Google AI API Key", type="password") if google_api_key: genai.configure(api_key=google_api_key) st.session_state.gemini_model = genai.GenerativeModel(gemini-pro) else: st.info("Please add your Google AI API key to continue.", icon="🗝️") st.stop()
...
Chat input
if prompt := st.chat_input("What is up?"): # Add user message to chat history st.session_state.messages.append({"role": "user", "content": prompt}) with st.chat_message("user"): st.markdown(prompt)
...
st.markdown(""" <style> * { direction: rtl; text-align: right; } body { background-color: #f5f5f5; } .stButton > button { width: 100%; background-color: #0066cc; color: white; padding: 10px; border-radius: 5px; font-weight: bold; } .stButton > button:hover { background-color: #0052a3; } .success-message { background-color: #d4edda; color: #155724; padding: 15px; border-radius: 5px; border-left: 4px solid #28a745; } .warning-message { background-color: #fff3cd; color: #856404; padding: 15px; border-radius: 5px; border-left: 4px solid #ffc107; } .danger-message { background-color: #f8d7da; color: #721c24; padding: 15px; border-radius: 5px; border-left: 4px solid #dc3545; } .info-card { background-color: #e7f3ff; border-right: 4px solid #0066cc; padding: 12px; border-radius: 5px; margin: 10px 0; } .user-message { background-color: #0066cc; color: white; padding: 12px; border-radius: 5px; margin: 8px 0; text-align: right; } .bot-message { background-color: #e8f4f8; color: #333; padding: 12px; border-radius: 5px; margin: 8px 0; text-align: right; } .section-header { color: #0066cc; font-size: 20px; font-weight: bold; margin-top: 20px; margin-bottom: 10px; border-bottom: 2px solid #0066cc; padding-bottom: 10px; } </style> """, unsafe_allow_html=True)
...
تهيئة session state
if "teen_data" not in st.session_state: st.session_state.teen_data = None if "chat_history" not in st.session_state: st.session_state.chat_history = [] if "show_form" not in st.session_state: st.session_state.show_form = True

================== الواجهة الجانبية (Sidebar) ==================
with st.sidebar: st.image("https://img.icons8.com/color/200/000000/parenting.png", width=100) st.title("👨‍👩‍👧‍👦 مرشد المراهقة الآمنة ") st.write("---")
...
================== الشاشة الرئيسية ==================
if st.session_state.show_form: # ===== قسم إدخال البيانات ===== st.title("👨‍👩‍👧‍👦 مرشد المراهقة الآمنة") st.write("ملء المعلومات التالية قبل الانتقال إلى المحادثة:") st.write("---")
...
else: # ===== منطقة المحادثة ===== if st.session_state.teen_data: # عرض بطاقة المعلومات teen = st.session_state.teen_data col1, col2, col3, col4 = st.columns(4)
...
لا تقدم أي تشخيصات طبية (اكتئاب، توحد، إلخ)
إذا شعرت أن الموقف يتطلب تدخل متخصصين، قل: "⚠️ هذا الموقف يتطلب تدخل متخصصين - الرجاء التواصل مع مختص نفسي"
اقسم ردك إلى 4 أقسام بالضبط:
...
<div style="text-align: center; color: #888; font-size: 12px;"> 📧 هذا التطبيق مصمم لتقديم مشورة والدية مساعدة فقط، وليس بديلاً عن استشارة المتخصصين.<br> ⚕️ في حالات الطوارئ، الرجاء التواصل مع متخصص نفسي مباشرة. </div> """, unsafe_allow_html=True)
