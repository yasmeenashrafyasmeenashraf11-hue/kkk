...
with st.sidebar:
    ...
    gemini_api_key = st.text_input("🔑 Gemini API Key", type="password", placeholder="ادخل مفتاح Gemini API")
...
if st.session_state.show_form:
    ...
    # ===== منطقة المحادثة =====
    if st.session_state.teen_data and "gemini_api_key" in st.session_state:
        # تكوين Gemini
...
استجب باللغة العربية فقط، بأسلوب دافئ وتعاطفي."""
            ...
            try:
                with st.spinner("🤖 المدرب يفكر..."):
                    ...
                    for msg in st.session_state.chat_history[:-1]:  # جميع الرسائل ما عدا الأخيرة
                        if msg["role"] == "user":
                            chat_content += f"المستخدم: {msg['content']}\n"
                        else:
                            chat_content += f"المدرب: {msg['content']}\n"
                    
                    # إضافة الرسالة الأخيرة (الحالية)
        ...
        st.warning("⚠️ الرجاء إدخال مفتاح Gemini API في الشريط الجانبي")
